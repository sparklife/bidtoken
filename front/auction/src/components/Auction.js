...
import { createContract } from '../ethereum/auctionContract'
...
export class Auction extends Component {
  ...
  async getCampaign(address) {
    ...
    const beneficiary = await contract.methods.beneficiary
    const deadlineSeconds = await contract.methods.fundingDeadline
  ...
  postCampaignInterface() {
    if (this.state.campaign.state === this.ONGOING_STATE) {
      return <div>
        <Button type='submit' positive>Finalize Auction</Button>
      </div>
    }
